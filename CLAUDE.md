...
- `saintImages.js` — `SAINT_IMAGES` map of public-domain Wikimedia Commons portrait URLs keyed by saint ID (movable feast IDs omit the year suffix, e.g. `s-sacredheart`). Applied as an overlay in `entities.js` (not merged into `saintsData.js`, which keeps `image_url: ''` for every entry). **Wikimedia only serves a fixed set of hotlink thumbnail widths — `20/40/60/120/250/330/500/960/1280/1920/3840`px** (see mediawiki.org/wiki/Common_thumbnail_sizes); any other `NNNpx-` prefix (e.g. `480px-`) 400s on direct requests even though it renders fine in a browser tab that hit Wikipedia first. Always use one of the allowed widths when adding new image URLs.

...
- `saintImages.js` — static map of saint ID → Wikimedia Commons thumbnail URL (public-domain images for ~45 major saints). Applied as an overlay in `entities.js` at load time so `saintsData.js` stays clean. Movable feast IDs omit the year suffix (key `'s-sacredheart'`, not `'s-sacredheart-2026'`). Saints without an entry get `image_url: ''`. IDs follow the `s-MMDD` pattern; a `-vo` suffix is not a valid ID format and will never match. **Most URLs still use a `/480px-.../` thumbnail width, which Wikimedia now rejects with a 400** — only entries fixed so far (bump to `/500px-.../`, confirmed live) render correctly; the rest silently fail and the image is hidden via the `<img>` `onError` handler. Also exports `SAINT_IMAGE_POSITION`, a per-saint override map for the vertical crop anchor (`'center'` or a percentage string like `'13%'`) — most source paintings are portraits with the face near the top, so unlisted saints default to top-anchored, but tight headshots or full-body scenes need a custom value found by empirically test-cropping the source image (see `entities.js`'s `imagePosition()` helper for the resolution logic).
- `entities.js` — loads `saintsData.js` / `liturgicalData.js` via dynamic `import()` (module-level singleton promises, so each data file becomes a separate cacheable chunk), computes moveable feasts for the current year at module load, and exposes thin CRUD wrappers that merge moveable + static arrays per call:
  ...
  - Every saint object also gets an `image_position` field (a CSS `object-position` value, e.g. `'center top'` or `'center 13%'`) computed from `SAINT_IMAGE_POSITION` via the local `imagePosition()` helper — this runs in all three merge sites (`_movableSaints`, `saintsPromise`, `getSaintsForYear`), so keep them in sync if you touch one.
...
- `use-streak.jsx` — `useStreak()` returns `{ streak, best, milestone }`. On mount, compares today's date (`yyyy-MM-dd`) to `fiat_lux_streak_last_date`: a 1-day gap increments the streak, any other gap resets it to 1, same-day is a no-op — idempotent, so it's safe to call from multiple components (`Today.jsx` shows `streak` in the header once `>= 2`; `SettingsPage.jsx` shows `best` as a personal-record stat). Day-gap math uses date-fns `parseISO`, not `new Date(str)`, since bare `yyyy-MM-dd` parses as UTC in native `Date` and can off-by-one near midnight in negative-UTC timezones. `milestone` is non-null only on the mount that pushes the streak onto one of `[7, 30, 100, 365]` (never on same-day remounts, since those hit the early-return no-op) — `Today.jsx` watches it in a `useEffect` and fires a one-off `toast()` celebration; it's UI-agnostic by design so the hook itself never imports the toast component.
...
### Saint hero images (Today card + `SaintDetailModal.jsx`)

Both the Today tab's saint-of-the-day card and `SaintDetailModal.jsx`'s header banner render `saint.image_url` with `className="object-cover grayscale"` and `style={{ objectPosition: saint.image_position }}` — grayscale plus the per-saint crop anchor described under `saintImages.js` above. **This treatment is scoped to just these two hero banners** — `SearchPage.jsx`'s circular Browse avatars render the same `image_url` in full color with a plain center crop, so don't assume grayscale is applied everywhere `image_url` shows up.
